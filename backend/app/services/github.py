...
def _build_tree(flat_files: list[str]) -> list[dict[str, Any]]:
    ...
    def to_nodes(d: dict, prefix: str) -> list[dict[str, Any]]:
        ...
        for name, info in sorted(d.items(), key=lambda x: (not x[1]["__is_file"], x[0].lower())):
            ...
            if info["__is_file"]:
                nodes.append({"path": rel, "name": name, "is_dir": False, "size": None, "modified_at": None})
            else:
...
async def fetch_github_tree(url: str, source_id: str) -> dict[str, Any]:
    ...
    data = resp.json()
    md_paths = [item["path"] for item in data.get("tree", []) if item["path"].endswith(".md") and item["type"] == "blob"]
    return {
        "source_id": source_id,
        "root": {
            "path": "",
